...
def conv_layer(inpt, filter_shape, stride1=1, stride2=1):
    ...
    conv = tf.nn.conv2d(inpt, filter=filter_, strides=[1, stride1, stride2, 1], padding="SAME")
...
def residual_block(inpt, output_depth, down_sample, projection=False):
    ...
    if input_depth != output_depth:
        if projection:
            ...
            input_layer = tf.nn.max_pool(inpt, ksize=filter_, strides=filter_, padding='SAME')
