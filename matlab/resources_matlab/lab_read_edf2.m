...
% Adapted by Matteo Cesari, Ph.D. Student DTU Electrical Engineering, 2017
% Adapted by Andreas Brink-Kjaer, Ph.D. Student DTU Health Technology, 2020
function [data_c,header,cfg] = lab_read_edf2(filename,cfg,verbose)

if ~exist('cfg','var')
    cfg = struct('read_data',true);
elseif isempty(cfg)
    cfg = struct('read_data',true);
end
...
% Data read
if verbose
    disp('Initial dataread');
    toc;
end

% order annotations speed
if verbose
    tic;
end
...
while strcmp(hdr.channelname(m,1:15),'EDF Annotations')
    ...
    header.hdr.channels = header.hdr.channels - 1;
    header.hdr.channelname(end,:) = [];
    header.hdr.transducer(end,:) = [];
    header.hdr.physdime(end,:) = [];
    header.hdr.physmax(end,:) = [];
    header.hdr.physmin(end,:) = [];
    header.hdr.digimax(end,:) = [];
    header.hdr.digimin(end,:) = [];
    header.hdr.prefilt(end,:) = [];
    header.hdr.numbersperrecord(end,:) = [];
    % Get event 
    eventstmp = data((end - hdr.numbersperrecord(m) + 1):end,:);
    time_stamp = nan(1,hdr.records);
    data = data(1:end-hdr.numbersperrecord(m),:);
    for i = 1:hdr.records
        eventsall = typecast(int16(eventstmp(:,i)),'uint8')';
        tmp = find(eventsall == 20);
        numberstops = tmp(eventsall(tmp+1) == 0);
        eventsall(eventsall == 32) = 95;
        eventsall(eventsall == 20) = 32;
        eventsall(eventsall == 21) = 32;
        if numberstops > 0
            tmp = textscan(native2unicode(eventsall(1:numberstops(1))),'%s');
            tmp = tmp{1,1};
            time_stamp(i) = str2double(tmp(1,1))*header.samplingrate + 1;
        end
        if size(numberstops,2) > 1
            for j = 2:size(numberstops,2)
                tmp = textscan(native2unicode(eventsall(numberstops(1,j-1)+2:numberstops(1,j))),'%s');
                tmp = tmp{1,1};
...
% Order annotations
if verbose
    disp('Event structure completed');
    toc;
end
...
data_c = cell(m,1);
for mi = 1:m
    data_c{mi} = reshape(data(1:hdr.numbersperrecord(mi),:),[1 (hdr.numbersperrecord(mi)*size(data,2))]);
    data(1:hdr.numbersperrecord(mi),:) = [];
end
clearvars data

% Reshape to cells
if verbose
    disp('Data reshaped to cells');
    toc;
end

% Correct to contiguous data
if verbose
    tic;
end
...
%My scale data
sf = (hdr.physmax - hdr.physmin)./(hdr.digimax - hdr.digimin); % Calculating scalingfactor!
dc = hdr.physmax - sf.* hdr.digimax; % Offsetting measured data.

for k = 1:size(data_c,1)
    data_c{k} = data_c{k}*sf(k)+dc(k);
end
